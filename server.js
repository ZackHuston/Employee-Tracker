...
app.get('/api/employee-reviews', (req, res) => {
  const sql = `SELECT employee.employee_name AS employee, reviews.review FROM reviews LEFT JOIN employee ON reviews.employee_id = employee.id ORDER BY employee.employee_name;`;
  db.query(sql, (err, rows) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
